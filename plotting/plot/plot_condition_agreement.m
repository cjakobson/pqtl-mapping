function [] = plot_condition_agreement(dependency_directory,output_directory)
    ...
    hold on
    bar((rm_coherent(sort_idx)+yjm_coherent(sort_idx))./sum(~isnan(mapping_beta)))
    xticks(1:length(short_names))
    xticklabels(short_names(sort_idx))
    axis square
    xlim([0 length(short_names)+1])
    title('agreement with mapping')
    ylabel('fraction concordant')
    xlabel('condition')
    plot(xlim,[0.5 0.5],':r')
    for i=1:length(p_coherent)
        text(i,0.9,num2str(p_coherent(sort_idx(i))),'rotation',45)
    end
