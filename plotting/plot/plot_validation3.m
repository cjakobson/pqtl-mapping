...
function []=plot_validation3(gene_name,mutant_to_plot,dependency_directory,output_directory)
    ...
    input_data=readtable([dependency_directory '20240607_proteomics-data_validation_set3.xlsx']);
    ...
    title(gene_name)
    for i=1:2
        [h p]=ttest2(to_plot{i},to_plot{3});
        plot([i 3],[1.1*mean(v_mean([i 3])) 1.1*mean(v_mean([i 3]))],'-k')
        text((i+3)/2,1.2*mean(v_mean([i 3])),num2str(p))
    end
