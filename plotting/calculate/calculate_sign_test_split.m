function [v_mean,v_background,p_val,sign_sum1,sign_sum2] = ...
    ...
    %pick random set
    idx_to_use=randperm(length(orf_names),ceil(length(orf_names)/2));
    proteins_to_use=orf_names(idx_to_use);

    all_pqtl_data=all_pqtl_data(ismember(all_pqtl_data.protein,proteins_to_use),:);
